...
import { createLighting, createEnvironment, LAMP_INTENSITY } from './render/environment';
...
async function boot() {
  const hash = readHash();
  const ids = shipIds();
  if (!ids.length) throw new Error('no ships found under ships/');
  const id = hash.ship && ids.includes(hash.ship) ? hash.ship : ids.includes('asv-07') ? 'asv-07' : ids[0];

  const spec = await loadSpec(id);
  const canvas = $<HTMLCanvasElement>('#gl');
  const stage = createStage(canvas, spec.camera.near ?? 0.5, spec.camera.far ?? 8000);
  const { sun, lamp } = createLighting(stage.scene, stage.camera);
  const worldScale = spec.length_m / 300;
  const stars = createStars(stage.scene, worldScale);
  createPlanet(stage.scene, sun.position, worldScale);
  createEnvironment(stage.renderer, stage.scene, sun.position);
