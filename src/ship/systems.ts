 ...
 */
type SegmentsCtor = typeof LineSegments2;
type MaterialCtor = typeof LineMaterial;
...
class Route {
  ...
  constructor(points: Point[], material: LineMaterial, Segments: SegmentsCtor) {
    this.points = points;
    ...
    this.line = new Segments(geo, material);
...
/** Fat lines come from the node build when the node pipeline is on, and the WebGL addon otherwise. */
async function lineClasses() {
  return __GPU__
    ? {
      Segments: (await import('three/addons/lines/webgpu/LineSegments2.js')).LineSegments2 as unknown as typeof LineSegments2,
      Material: (await import('three/webgpu')).Line2NodeMaterial as unknown as typeof LineMaterial,
    }
    : {
      Segments: (await import('three/addons/lines/LineSegments2.js')).LineSegments2,
      Material: (await import('three/addons/lines/LineMaterial.js')).LineMaterial,
    };
}
...
export class Systems {
  ...
  static async create(ship: LoadedShip) {
    const { Segments, Material } = await lineClasses();
    return new Systems(ship, Segments, Material);
  }

  private constructor(private ship: LoadedShip, Segments: SegmentsCtor, Material: MaterialCtor) {
    ...
    for (const sys of ship.spec.systems ?? []) {
      ...
      for (const path of sys.paths) {
        ...
        const r = new Route(pts, mat, Segments);
        routes.push(r);
