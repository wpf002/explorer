 ...
 */
import { launch, newPage, TIMEOUT } from './browser.mjs';
...
export async function measureShips(root, ids) {
  ...
  try {
    for (const id of ids) {
      const page = await newPage(browser, { viewport: { width: 800, height: 500 } });
      const errors = [];
      page.on('pageerror', e => errors.push(String(e)));
      await page.goto(`http://localhost:5194/${(process.env.MEASURE_PATH ?? "ship/ID").replace("ID", id)}`);
      try {
        await page.waitForFunction(() => window.FLEET?.stats, null, { timeout: TIMEOUT });
